...
class Sample(TimeStampedModel):
    ...
    barcode = models.CharField(
        ...
        help_text="Unique scannable barcode. For plate wells, this is auto-generated as PlateBarcode-WellID."
    )
    ...
    box_ID = models.CharField(
        max_length=100, blank=True, null=True,
        help_text="Identifier for the plate or box where this sample is stored"
    )
    ...
    def save(self, *args, **kwargs):
        # For samples in a plate, auto-generate a barcode from the plate's barcode and well position.
        if self.container_type == 'plate' and self.box_ID and self.well_ID:
            # Only generate if the barcode hasn't been set manually.
            if not self.barcode:
                self.barcode = f"{self.box_ID}-{self.well_ID}"
...
class SequenceLibrary(Sample):
    ...
    ANALYSIS_TYPE_CHOICES = [
        ('WGS', 'Whole Genome Sequencing (Short-Read)'),
        ('WGL', 'Whole Genome Sequencing (Long-Read)'),
        ('MSS', 'Metagenomic Shotgun Sequencing'),
        ('MTS', 'Metatranscriptomic Sequencing'),
        ('CFS', 'Cell-Free DNA Sequencing'),
        ('Other', 'Other'),
    ]
    ...
    library_type = models.CharField(
        ...
        default='Nextera',
        help_text="The wet-lab kit or method used (e.g., Nextera, SMARTer)"
    )
    analysis_type = models.CharField(
        max_length=100,
        choices=ANALYSIS_TYPE_CHOICES,
        help_text="The intended bioinformatics analysis (e.g., WGS, MSS)",
        null=True,
        blank=True
    )
